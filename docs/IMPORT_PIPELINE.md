...
- 由导入中心自动创建的文档节点会记录到任务的 `autoCreatedNodeId`。自动文档节点和导入任务在 Admin 数据库内同事务提交，避免任务保存失败时残留孤儿知识树节点。失败任务被物理删除时，服务端会同步删除该自动文档节点，并按当前知识树重新聚合知识库 `docCount/vectorCount`；手工选择的既有目录节点不会被删除。
