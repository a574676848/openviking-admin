...
| `CSP_CONNECT_SRC`      | string | —                       | 否         | 生产环境 `Content-Security-Policy` 的 `connect-src` 额外白名单，支持空格或逗号分隔；Web Proxy 会在请求期按 `BACKEND_URL || NEXT_PUBLIC_BACKEND_URL` 派生对应的 `http(s)` 与 `ws(s)` 源，再追加本变量 |
