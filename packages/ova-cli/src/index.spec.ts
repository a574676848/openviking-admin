...
describe("ova cli", () => {
  ...
  it("应该支持 jsonl 输出", async () => {
    ...
    expect(stdoutWrite).toHaveBeenCalledWith(
      expect.stringContaining('"id":"knowledge.search"'),
    );
  });
  ...
  it("应该在只有 API Key 时使用 capability key 调用能力接口", async () => {
    ...
    expect(stdoutWrite).toHaveBeenCalledWith(
      expect.stringContaining("traceId: trace-api-key"),
    );
  });

  it("知识库命令应调用 capability 命名空间接口", async () => {
    setStateFile(
      buildStateFile({
        apiKey: "ov-sk-demo",
      }),
    ...
    global.fetch = jest.fn().mockResolvedValueOnce(
      createJsonResponse({
        data: {
          items: [{ id: "kb-1", name: "EPAAS", status: "active" }],
        },
        traceId: "trace-kb",
      }),
    ) as unknown as typeof fetch;

    await bootstrap(["kb", "list", "--output", "json"]);

    expect(global.fetch).toHaveBeenCalledWith(
      "http://localhost:6001/api/v1/capability/knowledge-bases",
      expect.anything(),
    );
    expect(stdoutWrite).toHaveBeenCalledWith(
      expect.stringContaining('"id": "kb-1"'),
    );
  });
  ...
  it("文档导入命令应调用 capability 命名空间接口", async () => {
      ...
      createJsonResponse({
        data: {
          item: {
            ...
            status: "pending",
            sourceType: "url",
            targetUri: "viking://resources/tenants/rag/kb-1/",
          },
        },
        traceId: "trace-doc",
      }),
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "http://localhost:6001/api/v1/capability/import-tasks/documents",
      expect.objectContaining({
        method: "POST",
      }),
    );
  ...
  it("应该通过 configure 保存 API Key profile", async () => {
    ...
    expect(stdoutWrite).toHaveBeenCalledWith(
      expect.stringContaining('"hasApiKey": true'),
    );
  });
  ...
  it("应该支持 credential-options", async () => {
    ...
    expect(stdoutWrite).toHaveBeenCalledWith(
      expect.stringContaining('"credentialType": "api_key"'),
    );
  });
  ...
  it("应该拒绝覆盖非对象类型的 JSON mcpServers 配置", async () => {
    ...
    await expect(
      bootstrap(["setup", "--editor", "claude", "--output", "json"]),
  ...
  it("应该通过 init 生成 capability 快照与提示词注入块", async () => {
    ...
    expect(stdoutWrite).toHaveBeenCalledWith(
      expect.stringContaining('"capabilityCount": 1'),
    );
  });
...
function findWritePath(target: string) {
  return mockWriteFileSync.mock.calls.find((call) =>
    normalizePath(call[0]).includes(target),
  );
}
