...
export async function handleKnowledgeTree(
    ...
    const response = await callApi(
        command === 'detail'
            ? `/api/v1/knowledge-tree/${encodeURIComponent(String(options.id ?? ''))}`
            : `/api/v1/knowledge-bases/${encodeURIComponent(String(options.kb ?? options.kbId ?? ''))}/tree`,
        {},
        options,
        store,
    );
    ...
    emitOutput(
        ...
        response,
        () =>
            [`traceId: ${String(response.traceId ?? '')}`, ...items.map((item) => `${String(item.id)} ${String(item.name)} ${String(item.vikingUri ?? '')}`)].join('\n'),
        items.map((item) => ({
            profile: profileName,
