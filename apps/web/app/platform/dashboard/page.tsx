  ...
  return (
    <div className="w-full min-h-full flex flex-col pb-10">
      <PlatformPageHeader
        ...
        actions={
          <PlatformStateBadge
            tone={loading ? "warning" : data?.health?.ok ? "success" : "danger"}
            className="px-4 py-2 text-[10px]"
          >
            <div
              className={`mr-3 h-1.5 w-1.5 animate-pulse ${loading ? "bg-[var(--warning)]" : data?.health?.ok ? "bg-[var(--success)]" : "bg-[var(--danger)]"}`}
            />
      ...
      {/* SWISS BENTO GRID */}
      <div className="grid grid-cols-1 gap-0 border-[var(--border-width)] border-[var(--border)] bg-[var(--border)] md:grid-cols-3">
        {/* 平台级任务总览 */}
        <PlatformMetric
          label="导入任务"
          value={
          ...
          className="min-h-[300px] border-r-[var(--border-width)] px-10 py-10 shadow-none"
        ...
        <PlatformPanel className="col-span-1 border-t-[var(--border-width)] shadow-none md:col-span-3">
          <PlatformFooterBar
