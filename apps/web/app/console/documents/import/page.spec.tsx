...
function changeValue(element: Element | null, value: string) {
  const target = element as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null;
  if (!target) {
...
async function chooseConsoleSelect(index: number, optionText: string) {
  const trigger = container.querySelectorAll('form button[type="button"]')[index] as HTMLElement | undefined;
  if (!trigger) {
...
describe("IngestionPage", () => {
  ...
  it("Git 导入缺少知识库时应直接阻止提交", async () => {
    getMock
      .mockResolvedValueOnce([{ id: "integration-1", name: "exeGitLab", type: "gitlab" }])
  ...
  it("Git 导入提交时不再要求 targetUri", async () => {
    ...
    expect(successMock).toHaveBeenCalledWith("任务创建成功：已启动 1 个处理进程");
    expect(pushMock).toHaveBeenCalledWith("/console/documents");
  });

  it("导入中心不再展示 WebDAV 同步选项", async () => {
    getMock
      .mockResolvedValueOnce([{ id: "integration-2", name: "obsidian", type: "webdav" }])
      .mockResolvedValueOnce([{ id: "kb-1", name: "ov记忆系统" }]);
  ...
  it("知识库已有节点时应允许选择目标节点并提交节点 vikingUri", async () => {
    getMock
      ...
      .mockResolvedValueOnce([{ id: "kb-1", name: "ov记忆系统" }])
      .mockResolvedValueOnce([
        { id: "node-1", name: "产品文档", vikingUri: "viking://resources/tenant-a/kb-1/node-1/" },
      ]);
