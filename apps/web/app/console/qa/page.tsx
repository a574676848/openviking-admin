...
export default function QaPage() {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      ...
      const answer =
        sources.length === 0
          ? "ERR_NO_DATA: 未在知识库中找到相关内容。"
          : `已成功召回 ${sources.length} 条知识切片，正在为您生成答案...`;
    ...
    <div className="flex h-[calc(100vh-6rem)] flex-col gap-6">
      ...
      <div className="flex min-h-0 flex-col gap-8 lg:flex-row">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {messages.length === 0 ? (
              <div className="flex h-full flex-col items-center justify-center text-center">
                <ConsoleEmptyState icon={Terminal} title="Awaiting Input" description="type a query or use a suggested prompt" className="py-10" />
              ...
              <div className="space-y-8">
                ...
                {loading && (
                  <div className="flex items-start">
                    <div className="flex gap-3 border-[3px] border-[var(--border)] bg-[var(--bg-card)] p-6 shadow-[6px_6px_0px_#000]">
        ...
        <aside className="w-full shrink-0 space-y-6 lg:w-80">
          ...
          {messages.length > 0 && (
            <ConsolePanel className="bg-[var(--warning)] p-6">
              ...
              <div className="space-y-3 font-mono text-xs font-black">
                <div className="flex items-center justify-between">
                  <span>总轮次 / TURNS</span>
                  <span className="bg-black px-2 py-0.5 text-white">{Math.floor(messages.length / 2)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>命中率 / HITS</span>
