...
import { Activity, Clock, Database, ShieldCheck, Layers } from "lucide-react";
...
export default function SystemPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError("");
    try {
      const sessionUser = await apiClient.get<SessionUser>('/auth/me');
      if (!sessionUser.hasCustomOvConfig) {
    ...
    <div className="w-full flex flex-col pb-10 min-h-full">
      <PlatformPageHeader
        ...
        subtitle={
          <div className="flex items-center gap-4">
            <span>底层引擎状态与知识图谱遥测</span>
            {lastRefresh ? (
              <span className="border-l border-[var(--border)] pl-4 opacity-70">
      ...
      {loadError && (
        <PlatformStatusPanel
          ...
          action={
            <PlatformButton type="button" tone="danger" onClick={() => void load()} className="px-4 py-2 text-[10px]">
              重试加载
            </PlatformButton>
          }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-[var(--border-width)] bg-[var(--border)] border-[var(--border-width)] border-[var(--border)] shadow-[var(--shadow-base)] mb-8">
         <PlatformSignalCard
           ...
           value={loading ? <ScrambleText text="探测中" /> : health?.ok ? "在线" : "离线"}
         ...
         <PlatformSignalCard
           ...
           value={<ScrambleText text={loading ? '---' : totalQueue.toLocaleString()} />}
           hint="Embedding / Semantic 队列"
           accent={totalQueue && totalQueue > 0 ? "var(--warning)" : "var(--success)"}
           valueClassName="text-4xl md:text-5xl tabular-nums"
           className={totalQueue && totalQueue > 0 ? "before:absolute before:left-0 before:top-0 before:h-full before:w-1 before:animate-theme-pulse before:bg-[var(--warning)]" : undefined}
         />
         ...
         <PlatformSignalCard
           ...
           value={<ScrambleText text={loading ? '---' : (vikingdb?.totalVectorCount?.toLocaleString() ?? '---')} />}
