...
export function assertSafeRuntimeConfig(env: NodeJS.ProcessEnv) {
  ...
  assertRequired(env, 'FRONTEND_URL', '生产环境缺少 FRONTEND_URL，用于 CORS 白名单。');
  assertRequired(env, 'OV_BASE_URL', '生产环境缺少 OV_BASE_URL。');
  assertRequired(env, 'OV_API_KEY', '生产环境缺少 OV_API_KEY。');

  if ((env.JWT_SECRET ?? '').trim().length < 32 || hasPlaceholder(env.JWT_SECRET!)) {
  ...
  if (
    ...
    throw new Error('生产环境 ENCRYPTION_KEY 不安全，请使用至少 32 位随机字符串。');
  }
  ...
  if (hasPlaceholder(env.OV_BASE_URL!) || hasPlaceholder(env.OV_API_KEY!)) {
    throw new Error('生产环境 OpenViking 连接配置仍是占位值，请改为真实地址和密钥。');
  }
}
